...
static const char* sReplacementCharacters[] =
	{ "£21£","£40£","£23£","£24£","£25£","£5e£","£26£","£2a£","£28£","£29£",
	  "£2d£","£2b£","£3d£","£7b£","£7d£","£5b£","£5d£","£5c£","£7c£","£3b£",
	  "£3a£","£27£","£22£","£3c£","£3e£","£2c£","£2e£","£2f£","£3f£","£60£",
	  "£7e£","£20£", NULL
	};
...
static const char* sReplacementWords[] =
	{ " ¥select "," ¥drop "," ¥create "," ¥delete "," ¥where "," ¥update ",
	  " ¥order "," ¥by "," ¥and "," ¥or "," ¥in "," ¥between "," ¥aliases ",
	  " ¥join "," ¥union "," ¥alter "," ¥functions "," ¥group "," ¥into ",
	  " ¥view ", NULL
	};
...
{
	// Because the £ symbol isn't allowed in a category but is a valid database,
	// characterwe'll use it as the escape character for illegal characters
	...
	// Just to make sure that reserved words aren't used, we'll prefix them with
	// the ¥ character for the same reasons that we used £ with bad characters
	i = 0;
...
void
DBCommand(CppSQLite3DB& db, const char* command, const char* functionname)
{
	if (!command)
...
CppSQLite3Query
DBQuery(CppSQLite3DB& db, const char* query, const char* functionname)
{
	if (!query)
