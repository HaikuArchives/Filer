 	...
 	fOriginalID(-1)
{
	...
	// Description
	fDescriptionBox = new AutoTextControl(BRect(0,0,1,1),"description","Description: ",
											NULL,new BMessage(M_DESCRIPTION_CHANGED),
	...
	fAddTest = new BButton(BRect(0,0,1,1),"addtestbutton","Add",new BMessage(M_ADD_TEST),
							B_FOLLOW_LEFT | B_FOLLOW_BOTTOM);
	fAddTest->ResizeToPreferred();
	fAddTest->MoveTo(10.0, fTestGroup->Bounds().bottom - 10.0 - fAddTest->Bounds().Height());
	...
	fAddAction = new BButton(BRect(0,0,1,1),"addactionbutton","Add",new BMessage(M_ADD_ACTION),
							B_FOLLOW_LEFT | B_FOLLOW_BOTTOM);
	fAddAction->ResizeToPreferred();
	fAddAction->MoveTo(10.0, fActionGroup->Bounds().bottom - 10.0 - fAddAction->Bounds().Height());
	...
	fCancel = new BButton(BRect(0,0,1,1),"cancelbutton","Cancel",new BMessage(M_CANCEL),
						B_FOLLOW_RIGHT | B_FOLLOW_BOTTOM);
	fCancel->ResizeToPreferred();
	fCancel->MoveTo(fOK->Frame().left - fCancel->Bounds().Width() - 10, 
					fOK->Frame().top);
	
	fHelp = new BButton(BRect(0,0,1,1),"helpbutton","Help…",new BMessage(M_SHOW_HELP),
						B_FOLLOW_LEFT | B_FOLLOW_BOTTOM);
	...
	ResizeTo(Bounds().Width(), fActionGroup->Frame().bottom + 15 + fOK->Bounds().Height());
	
	fDescriptionBox->MakeFocus();
}


RuleEditWindow::~RuleEditWindow(void)
{
}
...
RuleEditWindow::MessageReceived(BMessage *msg)
{
	switch(msg->what)
	{
		...
		case M_OK:
		{
			...
			{
				BAlert *alert = new BAlert("Filer","You need to add a description "
												"if you want to add this rule to the list.",
												"OK");
				alert->Go();
...
{
	TestView *view = new TestView(BRect(0,0,1,1),"test",test,B_FOLLOW_LEFT_RIGHT | B_FOLLOW_TOP);
	view->ResizeToPreferred();
	BRect r = view->GetPreferredSize();
	fTestGroup->ResizeBy(0,r.Height() + 10);
	
	if (fTestGroup->Bounds().Width() < r.Width() + 20)
		ResizeBy(r.Width() + 20 - fTestGroup->Bounds().Width(),0);
	...
	fActionGroup->MoveBy(0,r.Height() + 10);
	ResizeBy(0,r.Height() + 10);
...
void
RuleEditWindow::RemoveTest(void)
{
...
{
	ActionView *view = new ActionView(BRect(0,0,1,1),"action",action,B_FOLLOW_LEFT_RIGHT | B_FOLLOW_TOP);
	BRect r = view->GetPreferredSize();
	view->ResizeTo(fActionGroup->Bounds().Width() - 20, r.Height());
	fActionGroup->ResizeBy(0,r.Height() + 10);
	
	if (fActionGroup->Bounds().Width() < r.Width() + 20)
		ResizeBy(r.Width() + 20 - fActionGroup->Bounds().Width(),0);
	
	ActionView *last = (ActionView*)fActionList.ItemAt(fActionList.CountItems() - 1);
...
void
RuleEditWindow::RemoveAction(void)
{
	ActionView *view = (ActionView*) fActionList.RemoveItem(fActionList.CountItems() - 1);
	view->RemoveSelf();
...
BRect
RuleEditWindow::GetPreferredSize(void) const
{
	...
	// Base minimum size for boxes (including inside padding) of 20 each and outside
	// padding of 10 pixels above each box
...
void
RuleEditWindow::SendRuleMessage(void)
{
